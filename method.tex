...
これは次のように表示される。
\begin{flalign}
	c_{it} = \sum_{j \in N^\mathrm{in} } g(b_{ijt}, q_j) q_j+ \alpha_i
\end{flalign}
...
\begin{flalign}
	\max_{\vect{b}, q} G_{it} = \max_q q d_{it}(q) - \min_{\vect{b}} \vect{g}_{it}(\vect{b})^\T( \hat{\vect{q}}_t - \gamma \vect{o}_{i,t+1}  ) + \const
\end{flalign}
