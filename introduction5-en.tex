...
The typical betrayal is hidden information. That is, buyer prefers to underevaluate value of observation for paying nothing, and the seller prefers to sell noise instead of observation. %TODO 何か処理をすること前提
...
%Are there truthful mecchanism which makes the agents to declare true information in an environment ?
\begin{center}
%{\em How to make communicating agents to declare true information in a patially-observed environment without an administrator?}
%{\em How to make communicating agents to declare true information in a patially-observed environment?}
%{\em How to make communicating agents to declare true information without an administrator?}
%{\em How to guarantee communicating agents declare true information without an administrator?}
%{\em Are there any mechanisms to have communicating agents declare true information without an administrator?}
%{\em Is there any mechanism to have communicating agents declare true observation?}
%{\em Is there any mechanism to have communicating agents evaluate value of observation truthfully, without any administration?}
%{\em Is there any mechanism to have communicating agents emit valuable signal \\ without any administration?}
{\em Is there any mechanism to have communicating agents truthfully declare valuable observation \\ without any administration?}
\end{center}
% 自律的なエージェントに正しい情報を申告させるためにはどうしたらよいか。

To address the question, we first make the agents evaluate value of observation truthfully
with {\em envy-free auction} \citep{guruswami2005profit} in economics.
If the value is evaluated precisely, the agents send valuable observation autonomously since the observation is more valuable than noise.
%to MARL, and makes agents to declare true information without administrator.
Envy-free auction is a basic mechanism in a digital goods auction, an auction mechanism for copyable goods such as books and music.
...
% 観測の価値が正しく評価されれば、ノイズよりは価値のある情報を流した方がよいため、エージェントは自律的に価値の高い情報を流すようになる。

%With this mechanism, 

% この辺ちょっと甘い
We combine the mechanism of auction into an existing method which optimizes communication with backpropagation \citep{sukhbaatar2016learning}.
...
we create a model which regards a single unit as an agent for simplicity, and name it {\em Neuron as an Agent} (NaaA).
