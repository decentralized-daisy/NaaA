...
NaaA can be used to not only multi-agent RL but training of network.
Typical training algorithms of a neural network such as RMSProp \citep{tieleman2012lecture} and Adam \citep{kingma2014adam} are based on stochastic gradient descent (SGD), 
and the optimization is performed sequentially.
Hence, the problem can be interpreted as the problem to update the state (i.e., weight) to the goal which is manimization of expected likelihood.

The learning can be accelarated by applying NaaA to the optimizer.
We name what applying NaaA to SGD as {\em Adaptive DropConnect} (ADC), 
which is eventually combination of DropConnect \citep{wan2013regularization} and Adaptive DropOut \citep{ba2013adaptive}.
We introduce ADC herein as one of the application of NaaA.

ADC uses NaaA for supervised optimization problem with several revises.
Firstly, an environment has an input state such as image, the agent should update its parameters to maximize its reward obtained from the criterion calculator.
The criterion calculator give batch-likelihood as the reward to the agent.
The agent is the classifier which updates its weights to maximize the reward from criterion calculator.
The weights are recorded as an internal state.
As counterfactual return $o_{ijt}$, we employed heuristic that uses absolute value of the weight $|w_{ijt}|$, 
same technique as the one which used by Adaptive DropOut.
The reason why we use absolute value of weights is the update amount for the magnitude of error of output of units is in proportion to $|w_{ijt}|$.

The algorithm is shown in Algorithm 2.
Since the algorithm is quite simple, the implementation can be performed easily,
and hence it can be widely applied for the most of general deep learning problem such as 
...
\begin{algorithmic}[1]
	\FOR{ $t=1$ \TO $T$ }
		\STATE Compute bidding price for every edges: \textbf{for} $(v_j, v_i) \in \edges$ \textbf{do} \
		$b_{ijt} \leftarrow Q^{\pi_i}( \vect{s}_{it}, \vect{e}_j) - Q^{\pi_i}( \vect{s}_{it}, \vect{0})$ 
		\STATE Compute asking price for every nodes: \textbf{for} $\unit \in \units$ \textbf{do} \
		$\opt{q}_{it} \leftarrow \argmax_{q \in [0, \infty)} q d_{it}(q).$
		\FOR{$(v_i, v_j) \in \edges$}
				\STATE Compute allocation: $g_{jit} \leftarrow H(b_{jit} - \opt{q}_{it})$ 
				\STATE Compute price the agent should pay: $\rho_{jit} \leftarrow g_{jit} \opt{q}_{it}$ 
		\ENDFOR
		\STATE Make payment: \textbf{for} $\unit \in \units$ \textbf{do} \
		$R_{it} \leftarrow \sum_{j \in N^\mathrm{out}_i} \rho_{jit} 
				- \sum_{j \in N^\mathrm{in}_i} \rho_{ijt},$
		\STATE Make shipment: \textbf{for} $\unit \in \units$ \textbf{do} \
		...
		\FOR{$\unit \in \units$} 
			...
			\STATE Update $Q^{\pi_i}$ under the manner of $Q$-learning by calculating time difference (TD)-error 
		\ENDFOR
	\ENDFOR
\end{algorithmic}
\end{algorithm}
...
\begin{algorithmic}[1]
	\FOR{ $t=1$ \TO $T$ }
		...
		\STATE Compute $\vect{h}_t$ with making shipment:
