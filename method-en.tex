...
A $\agent$ environment was comprised of an environment where the multi-agent system itself touched a 
set of agents to which $\agent$ directly connects: $\{v_i \in \agents | i \in \friends\}$.
The former was defined as the external environment, whereas the latter was termed the internal environment.
$\agent$ received rewards from both environments.
...
\item (Conservation) 	% Energy Conservation
	...
	Hence, the summation of rewards which $\agents$ (receive both internal and external environment $\reward$) are equivalent to the reward $R_t^{\mathrm{ex}}$, which the entire multi-agent system receives from the external environment: $\sum_{i=1}^n R_{it} = \sum_{i=1}^n R_{it}^{\mathrm{ex}}$.
\item (Trade) 
...
%================================================================
The external reward agent $\agent$ received at time step $t$ was denoted as $R_{it}^\mathrm{ex}$, where $\sum_{i=1}^n R_{it}^\mathrm{ex} = R_t^{\mathrm{ex}}$ holds.
