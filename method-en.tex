...
Some agents receive rewards from the environment, and they distribute these to other agents as incentives for giving precise information.
Rewards are limited, so if an agent distributes $\rho$ rewards, the reward of that agents is reduced by $\rho$, as it would be with currency.
...
A typical artificial neural network can be represented as a directed graph $\neuralnet = (\units, \edges)$ between units, where $\units = \{\unitAt{1}, \dots, \unitAt{N}\}$ is a set of the units and $\edges \subset \units^2$ is a set of edges representing the connections between two units.
If $(\unit, \unitAt{j}) \in \edges$, then connection $\unit \rightarrow \unitAt{j}$ holds, indicating that $\unitAt{j}$ observes the activation of $\unit$.
Here, the activation of unit $\unit$ at time $t$ was denoted as $x_{it} \in \Real$.
Additionally, the set of units that unit $i$ connects to was designated to be $\followers = \{j | (\unit, \unitAt{j}) \in \edges \}$ and the set of units that unit $i$ is connected from was $\followees = \{j | (\unitAt{j}, \unit) \in \edges \}$.
Furthermore, $\friends = \followees \cup \followers$.

The proposed NaaA method interprets $\unit$ as an agent, making $\neuralnet$ a multi-agent system.
A $\unit$ environment was comprised of an environment where the multi-agent system itself touched a 
set of units to which $\unit$ directly connects: $\{v_i \in \units | i \in \friends\}$.
The former was defined as the external environment, whereas the latter was termed the internal environment.
$\unit$ received rewards from both environments.
The following assumptions were added to the $\unit$ characteristics:
\begin{enumerate}
\renewcommand{\labelenumi}{N\arabic{enumi}:}
\item (Selfishness) 
	The utility each unit $\unit$ wants to maximize is its own return (cumulative discounted reward):
	$G_{it} = \sum_{k=0}^T \gamma^k \rewardAt{i,t+k}$.
\item (Conservation) 	% Energy Conservation
	The summation of internal rewards over all $\units$ equals 0.
	Hence, the summation of rewards which $\units$ (receive both internal and external environment $\reward$) are equivalent to the reward $R_t^{\mathrm{ex}}$, which the entire multi-agent system receives from the external environment: $\sum_{i=1}^n R_{it} = \sum_{i=1}^n R_{it}^{\mathrm{ex}}$.
\item (Trade) 
	A unit $\unit$ will receive an internal reward $\rho_{jit}$ from $\unitAt{j} \in \units$ in exchange for an activation signal $x_i$
	before transferring this signal to the unit. Simultaneously, $\rho_{jit}$ will be subtracted from the reward of $v_j$.
\item (NOOP) 
	$\unit$ can select NOOP (no operation), for which the return is $\delta > 0$, as an action.
	In NOOP, the unit inputs and outputs nothing.
\end{enumerate}
...
%================================================================
The external reward unit $\unit$ received at time step $t$ was denoted as $R_{it}^\mathrm{ex}$, where $\sum_{i=1}^n R_{it}^\mathrm{ex} = R_t^{\mathrm{ex}}$ holds.
From N3, the reward $R_{it}$ received by $\unit$ at $t$ can be written as:
\begin{flalign}
...
The unit $\unit$ maximized its cumulative discounted profit, $G_{it}$, represented as:
\begin{flalign}
...
$G_{it}$ was approximated with the value function $V_i^{\pi_i} (s_{it}) = \Expect{\pi_i}{ G_{it} \mid s_{it}}$ where $s_{it} \in \Observations$.
...
The inequality $R_{it} > 0$ (i.e., $r_{it} > c_{it}$) indicates that the unit in question gave additional value to the obtained data.
The unit selected the NOOP action because $V_i^{\pi_i} (s_{it}) \leq 0 < \delta$ if $R_{it} \leq 0$ for all $t$ (because of N4).
...
%(TODO: Bad writing.)
If the optimization problem of NaaA is itself naively optimized, a trivial solution is obtained in which the internal rewards converge at 0, and all units (excepting output units) select NOOP as their action.
This phenomenon occurs regardless of the network topology $\neuralnet$, as no nodes are incentivized to send payments $\rho_{ijt}$ to other units.
...
To make the agents to evaluate other agents' value truthfully, the proposed objective function borrows its idea from the digital goods auction theory \cite{guruswami2005profit}.
This auction theory is a part of mechanism design intended to unveil the true price of goods.
...
Therefore, the positive side was named $\unit$'s {\em price}, and denoted as $q_{it}$.

%TODO: Describe for allocation

The envy-free auction process is shown in the left section of Figure \ref{fig:double}, displaying the negotiation process between one unit sending an activation (defined as a seller), and a group of units buying the activation (defined as buyers).
First, a buyer places a bid with the unit at a bidding price $b_{jit}$ (\textbf{1}).
Next, the seller selects the optimal price $\opt{q}_{it}$ and allocates the activation (\textbf{2}).
...
The seller sends the activation $x_i$ only to the allocated buyers (\textbf{4}).
Buyers who do not receive the activation approximate $x_i$ with $\Expect{\pi}{x_i}$.
This negotiation is performed at each time step in RL.
...
\end{flalign}
where $d_{it} = \sum_{j \in N^\mathrm{out}_i} g_{jit}$ is some number of units for which the $q_{it}$ bidding price is greater than or equal to $q_{it}$ (designated as the demand).
...
%========================================
\textbf{Cost}:
Cost is defined as an internal reward that one unit pays to other units.
...
If $b_{ijt}$ is minimized and $b_{ijt} = 0$, the activation purchase fails and the reward obtained by the seller unit from the connected buyer units is lower in the future.
...
\end{flalign}
where $\vect{o}_{it}$ is an implemented parameter (the difference between two $v_i$ returns) regardless of whether $x_i$ is observed.
...
\textbf{Right}: Unit price determination curve. Unit revenue is a product of monotonically decreasing demands and prices. The price that maximizes revenue is the optimal price.
}
\label{fig:double}
\end{figure*}


The optimization problem is, therefore, presented below.
\begin{flalign}
...
\end{thm}
This proof is shown in the Appendix.

This implies units should only consider their counterfactual returns!.
When $\gamma = 0$ it is equivalent to a case without auction. Hence, the bidding value is raised if each unit considers their long-time rewards. 
Consequently, when the NaaA mechanism is used units behave as if performing valuation for other units, 
and declare values truthfully.

Under these conditions, the following corollary holds:
\begin{coro}\label{coro:optimal-bidding}
	The Nash equilibrium of an envy-free auction is $(\vect{b}_{it}, q_{it})$ is $(\vect{o}_{it}, \argmax_{q} q d_{it}(q))$.
\end{coro}.
...
$Q$-learning was used to predict $\vect{o}_t$ in this paper.
